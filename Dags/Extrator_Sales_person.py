...
dag = DAG(
    'BI_SalesPerson', default_args=default_args, schedule=timedelta(days=1))
...
def extrator_csv():
    ...
    query = 'SELECT [BusinessEntityID],[TerritoryID],[SalesQuota],[Bonus],[CommissionPct],[SalesYTD],[SalesLastYear],[rowguid],[ModifiedDate] FROM [AdventureWorks2019].[Sales].[SalesPerson]'
    ...
    df.to_csv(r'/tmp/SalesPerson.csv', index=False,sep='|')
    conn.close()
...
def Upload_GCS():
    ...
    blob = bucket.blob('SalesPerson.csv')
    #print(buckets)

    #upload do arquivo CSV para o GCS
    blob.upload_from_filename(r'/tmp/SalesPerson.csv')
...
load_BigQuery = GCSToBigQueryOperator(
    ...
    source_objects=['SalesPerson.csv'],
    destination_project_dataset_table='portiolio.DW.d_person',
