...
dag = DAG(
    'BI_Sales_Territory', default_args=default_args, schedule=timedelta(days=1))
...
def extrator_csv():
    ...
    query = 'SELECT [TerritoryID],[Name],[CountryRegionCode],[Group],[SalesYTD],[SalesLastYear],[CostYTD],[CostLastYear],[rowguid],[ModifiedDate] FROM [AdventureWorks2019].[Sales].[SalesTerritory]'
    ...
    df.to_csv(r'/tmp/SalesTerritory.csv', index=False,sep='|')
    conn.close()
...
def Upload_GCS():
    ...
    blob = bucket.blob('SalesTerritory.csv')
    #print(buckets)

    #upload do arquivo CSV para o GCS
    blob.upload_from_filename(r'/tmp/SalesTerritory.csv')
...
load_BigQuery = GCSToBigQueryOperator(
    ...
    source_objects=['SalesTerritory.csv'],
    destination_project_dataset_table='portiolio.DW.d_sales_terrytory',
