...
dag = DAG(
    'BI_CreditCard', default_args=default_args, schedule=timedelta(days=1))
...
def extrator_csv():
    ...
    query = 'SELECT [CreditCardID],[CardType],[CardNumber],[ExpMonth],[ExpYear],[ModifiedDate] FROM [AdventureWorks2019].[Sales].[CreditCard]'
    ...
    df.to_csv(r'/tmp/CreditCard.csv', index=False,sep='|')
    conn.close()
...
def Upload_GCS():
    ...
    blob.upload_from_filename(r'/tmp/CreditCard.csv')
...
load_BigQuery = GCSToBigQueryOperator(
    ...
    source_objects=['CreditCard.csv'],
    destination_project_dataset_table='portiolio.DW.d_CreditCard',
